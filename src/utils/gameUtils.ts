...
import { BOARD_SPACES, GAME_ACTIONS } from "./gameConstants";
...
// activeCategories is a Record<string, boolean>, e.g., { DARE: true, DRINK: true, ... }
export const getRandomAction = (activeCategories: Record<string, boolean>): any => {
  // Get the categories that are turned on and exist in GAME_ACTIONS
  const availableTypes = Object.keys(activeCategories).filter(
    (cat) => activeCategories[cat] && GAME_ACTIONS[cat] && GAME_ACTIONS[cat].length
  );
  if (availableTypes.length === 0) return null;

  // Pick a random type (category)
  const randomType = availableTypes[Math.floor(Math.random() * availableTypes.length)];
  const actionPool = GAME_ACTIONS[randomType];
  const idx = Math.floor(Math.random() * actionPool.length);
  const picked = actionPool[idx];

  // Special handling for CHOICE and POSITION
  if (randomType === "CHOICE") {
    return { type: randomType, options: picked };
  }
  if (randomType === "POSITION") {
    return { type: randomType, ...picked };
  }
  // For DARE, DRINK, QUESTION, EVENT (all strings)
  return { type: randomType, text: picked };
};
